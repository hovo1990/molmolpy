...
def rmsd_custom(V, frame=0):
    ...
    v = V[frame]
    for w in W:
        curr_dist = sum([(v[i]-w[i])**2.0 for i in range(D)])
        # distance.append(np.sqrt(curr_dist))
        distance.append(curr_dist)
    return distance


def vector_distance(V, frame=0):
    """
    Calculate Root-mean-square deviation from two sets of vectors V and W.
    """
    D = len(V[0])
    ...
    #     curr_dist = np.sqrt(sum([(W[i]-W[i+1])**2.0]))
    #     # distance.append(np.sqrt(curr_dist))
    #     distance.append(curr_dist)
    ...
    i = 0
    curr_dist = np.sqrt(
        (((W[i][0] - W[i][0]) ** 2.0) + ((W[i][1] - W[i][1]) ** 2.0) + ((W[i][2] - W[i][2]) ** 2.0)))
    # distance.append(np.sqrt(curr_dist))
    distance.append(curr_dist)

    for i in range(len(W)-1):
        curr_dist = np.sqrt((((W[i+1][0]-W[i][0])**2.0)+((W[i+1][1]-W[i][1])**2.0)+((W[i+1][2]-W[i][2])**2.0)))
        # distance.append(np.sqrt(curr_dist))
        distance.append(curr_dist)
